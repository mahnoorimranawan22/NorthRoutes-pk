...
export default function Tours() {
  ...
  useEffect(() => {
    const fetchTours = async () => {
      try {
        setLoading(true);
        const res = await toursAPI.getAll({ search: search || undefined });
        setTours(res.data.data || res.data.tours || res.data);
      } catch {
        // Fallback to mock data if API is unavailable
        setTours(MOCK_TOURS.map(t => ({ ...t, slug: t.slug || t.id })));
        setError(null);
      } finally {
        setLoading(false);
      }
  ...
  const displayTours = tours.length > 0 ? tours : MOCK_TOURS.map(t => ({ ...t, slug: t.slug || t.id }));
