...
// Clear any existing model registrations to avoid OverwriteModelError
function clearModels() {
  Object.keys(mongoose.models).forEach((key) => {
    delete mongoose.models[key];
  });
  Object.keys(mongoose.connection.collections).forEach((key) => {
    delete mongoose.connection.collections[key];
  });
}
...
function testTourSchema() {
  ...
  try {
    ...
    invalidTour.validateSync();
    assert(false, "Should throw on missing required fields");
  } catch (e) {
    assert(e.errors.destinations !== undefined, "Validates missing destinations");
    assert(e.errors.pickupPoints !== undefined, "Validates missing pickupPoints");
    assert(e.errors.itinerary !== undefined, "Validates missing itinerary");
    assert(e.errors.pricePerPerson !== undefined, "Validates missing pricePerPerson");
  }
...
function testHotelSchema() {
  ...
  // Missing required
  try {
    const invalid = new Hotel({ name: "Test" });
    invalid.validateSync();
    assert(false, "Should throw on missing required fields");
  } catch (e) {
    assert(e.errors.destination !== undefined, "Validates missing destination");
    assert(e.errors.totalRooms !== undefined, "Validates missing totalRooms");
    assert(e.errors.startingPricePerNight !== undefined, "Validates missing startingPricePerNight");
  }
...
function testRoomSchema() {
  ...
  // Missing required
  try {
    const invalid = new Room({ type: "Test" });
    invalid.validateSync();
    assert(false, "Should throw on missing required fields");
  } catch (e) {
    assert(e.errors.hotel !== undefined, "Validates missing hotel");
    assert(e.errors.bedType !== undefined, "Validates missing bedType");
    assert(e.errors.sizeSqm !== undefined, "Validates missing sizeSqm");
    assert(e.errors.maxGuests !== undefined, "Validates missing maxGuests");
    assert(e.errors.pricePerNight !== undefined, "Validates missing pricePerNight");
    assert(e.errors.totalRooms !== undefined, "Validates missing totalRooms");
  }
...
function testBookingSchema() {
  ...
  assert(comboBooking.priceBreakdown.balanceDue === 167800, "Balance due calculated correctly");

  // Test 4: Missing required fields
  try {
    const invalid = new Booking({ bookingRef: "TEST-001" });
    invalid.validateSync();
    assert(false, "Should throw on missing required fields");
  } catch (e) {
    assert(e.errors.user !== undefined, "Validates missing user");
    assert(e.errors.bookingType !== undefined, "Validates missing bookingType");
    assert(e.errors.guests !== undefined, "Validates missing guests");
    assert(e.errors.totalGuests !== undefined, "Validates missing totalGuests");
    assert(e.errors.priceBreakdown !== undefined, "Validates missing priceBreakdown");
  }
...
function testUserSchema() {
  ...
  try {
    const invalid = new User({ name: "Test" });
    invalid.validateSync();
    assert(false, "Should throw on missing required fields");
  } catch (e) {
    assert(e.errors.email !== undefined, "Validates missing email");
    assert(e.errors.password !== undefined, "Validates missing password");
  }
  ...
  assert(paths.includes("emergencyContact"), "Schema has 'emergencyContact' path");
...
function testModelExports() {
  ...
  assert(typeof models.Tour === "function", "Tour model exported");
  assert(typeof models.Hotel === "function", "Hotel model exported");
  assert(typeof models.Room === "function", "Room model exported");
  assert(typeof models.Booking === "function", "Booking model exported");
  assert(typeof models.User === "function", "User model exported");
  assert(models.Tour.modelName === "Tour", "Tour model name is 'Tour'");
  assert(models.Hotel.modelName === "Hotel", "Hotel model name is 'Hotel'");
  assert(models.Room.modelName === "Room", "Room model name is 'Room'");
  assert(models.Booking.modelName === "Booking", "Booking model name is 'Booking'");
  assert(models.User.modelName === "User", "User model name is 'User'");
